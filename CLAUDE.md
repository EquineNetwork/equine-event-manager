...
> ## 🚦 START-OF-SESSION — READ THESE FIRST (especially on a new machine / laptop)
> This `CLAUDE.md` is the only file auto-loaded each session. Before doing anything else:
> 1. **Read `ROADMAP.md`** — the "🔖 SESSION HANDOFF" block at the very top tells you exactly
>    where the last session stopped and what to pick up next (currently: tasks #229 → #234 → #235).
> 2. **First time on this machine? Read `docs/LAPTOP-SETUP.md`** — the step-by-step to mirror the
>    desktop dev environment (clone repo + Local site Export/Import + symlink the plugin so edits
>    preview live at `http://en-event-manager.local`). Do this before trying to run/preview anything.
> 3. **Heads-up: per-machine memory does NOT travel with the repo.** The desktop's saved memory
>    (e.g. "don't test on reservation 5990 — its RV map is corrupted"; test on **NTR 6519**) lives
>    under `~/.claude/` on the desktop, not in git. The critical do-not-touch facts are mirrored
>    into the ROADMAP handoff block so they're not lost — trust the ROADMAP, not an empty memory.
> 4. Then continue with the workflow + conventions below as normal.
...
Solo non-coder owner (Whitney). You implement; I review. Make edits surgical and incremental — no sweeping changes without showing me first. Hide the WP admin bar and sidebar on all mockups.

## Layout rules (LOCKED)

- Editor pages use a single-card accordion layout.
- EXCEPTION: event editor pages use CSS grid (`1fr 280px`) with a sticky right rail (Publish + Link Reservation cards, no toggle).
...
**Token value changes require grep audit for hardcoded literals in ALL stylesheets — admin-legacy.css included (C7.X.18 lesson — Issue A2, hardened by C7.X.19):** When a design token like `--eem-radius` changes value (e.g. 4px → 3px), the change ONLY cascades to selectors that reference the token via `var(--eem-radius)`. Hardcoded literal values (e.g. `border-radius: 4px`) are left behind silently. After ANY token value change, run:
```bash
grep -rn "border-radius:" assets/css/ | grep -v "var(--eem-radius" | grep -v "border-radius: 0"
```
and convert every appropriate hardcoded match to the token. **This grep MUST cover admin-legacy.css.** C7.X.18's A2 sweep only checked admin.css for `4px`; it missed a `12px !important` in admin-legacy.css block 6 (the cascade-winning form-control block). The result was `.eem-zone-name-input` / `.eem-repeat-input` computing 12px at runtime even after C7.X.17 converted all other blocks to the token — not caught until C7.X.19 browser verify. The single-grep command above already hits both files (`assets/css/`), but confirm both are included in the output before considering the sweep done. Intentional exceptions (pill shapes = `999px`, circles = `50%`, card containers = `var(--eem-radius-lg)`) stay but MUST have an inline comment explaining why they're not the standard token. Going forward, all new CSS for form-control / button-family elements MUST use `var(--eem-radius)`, never a hardcoded literal. If a different radius is intentionally needed, define a new semantic token (`--eem-radius-pill`, `--eem-radius-circle`) rather than hardcoding.
...
**Source-presence assertions are insufficient for runtime/computed bugs — browser self-verify is mandatory (C7.X.19 lesson):** When a reported symptom is phrased in runtime terms (computed value, dynamically-added class, click reachability), a source-presence grep that finds the fix in the source file is **not** evidence the bug is fixed. Three classes of runtime/computed bugs that source-grep cannot verify:

1. **Cascade-winner mismatch** — C7.X.19 Issue 1: `.eem-zone-name-input` / `.eem-repeat-input` computed `border-radius: 12px` at runtime. The correct `var(--eem-radius)` was present in blocks 1–5; block 6 was the cascade winner and had `12px`. Grepping for `var(--eem-radius)` in the source returned true (blocks 1–5 existed); the cascade winner was a different block in the same file. Source-presence smoke cannot determine which `!important` wins.

2. **Dynamically-added class never fires** — C7.X.19 Issue 2: `eem-row-menu-wrap--flip-up` was present in CSS and the JS classList.add call was present in source, but the class was NEVER added at runtime. The bug was that the boundary comparison used `window.innerHeight` instead of the actual clipping container (`.eem-page-wrap`, `overflow:hidden`). Grepping for `classList.add('eem-row-menu-wrap--flip-up')` confirmed the code existed — but the condition was always false at runtime.

3. **Click unreachability due to container clipping** — C7.X.19 Issue 3 (consequence of Issue 2): "Delete Permanently" button existed in the DOM, had a handler, but was physically hidden behind the clipping container boundary. Source-presence checks on the button and handler both passed; the button was unreachable because of a scroll/overflow layout condition.

**Operational rule:** Any fix that targets a runtime/computed symptom (wrong computed value, class that should be applied by JS, modal that should open when clicking) MUST be verified by loading the page in a browser and observing the runtime behavior directly. The smoke test for these fixes is allowed to assert source-shape only when the smoke's header explicitly notes: *"Source-presence assertions only — mandatory browser self-verify required for computed/runtime claims."* Never report a runtime/computed symptom as fixed without this browser step.
...
> **V1 SCOPE (2026-06-09, Whitney): the admin Collect Payment "Charge Card" tab IS v1. ✅ DONE + LIVE-VERIFIED (2026-06-12).** Customer-side Auth.net charge + refunds already worked; the admin Collect Payment "Charge Card" dispatch for Auth.net was wired parallel to the Stripe path (2.7.x). **Two live test charges run end-to-end through the admin path on 2026-06-12 — both succeeded.** This was the last v1 launch gate; it is now cleared. v1 is feature-complete. (Historical: was "blocked on credentials, not code" until Whitney's Auth.net API login + transaction key landed.)
...
- **Facility Layout Templates — V2 (2026-06-12, Whitney).** Save a venue's stall/RV grid layout as a reusable, named **template tied to a venue** so recurring annual events at the same complex don't rebuild the (often large/complex) map from scratch. Flow: (a) persist **venue names** to their own table/CPT — sourced from whichever event source supplies them (GEMS, TEC, or Native Events); (b) when building a stall grid, offer **"Save as template for {venue}"**; (c) next year, when creating a new reservation at that venue, **select a prior-year template** and **clone it into the new reservation** — edits/additions apply to the clone, NEVER mutating the original template (copy-on-use, not reference). Key design points: venue identity must normalize across the 3 event sources; template = the `_en_stall_rows`/map structure + RV zones + blocked-stall config; versioning/lineage ("based on 2025 template") is nice-to-have. High value for big recurring complexes. Scope TBD at v2 kickoff.
- **Notifications — V2 (2026-06-12, Whitney; scope refined). DEDICATED PAGE + audience builder.** A first-class **Notifications** admin page (top-level Event Manager submenu) where the admin: (1) **picks an event**; (2) builds an **audience** via an Include/Exclude/Payment model; (3) composes subject + body; (4) sends. The audience model (locked — derived from Whitney's examples "everyone who entered #9.5 Division" + "RV buyers but not stall customers"):
  - **Include** (base segment): All customers · Stall customers · RV customers · Add-on customers · a specific **Division's entrants** (dropdown of that event's divisions) · Group customers.
  - **Exclude** (optional segment, same option set + "none") — recipients = Include − Exclude (by distinct email). This is what makes "RV but not stall" work.
  - **Payment** (optional, applied on top): All · Paid only · Unpaid only.
  - **Live recipient count** updates as the audience is built.
  Recipients resolved authoritatively from ORDERS (not stall/RV-table notes) so division-only customers are included; division segments use the `wp_eem_division_entries` ledger. Send = batched/throttled step endpoint (reliability on large events; reuse the bulk-send-link queue pattern), Emogrifier-inlined via `EEM_Mailer`, logged to the activity log (audience description + sent/failed). A **send history** view lists past notifications from the activity log. The existing per-reservation Email Customers modal (`render_email_customers_modal`) is superseded by / folded into this page. Open at kickoff: saved reusable segments (nice-to-have), per-recipient personalization tokens, opt-out handling.
- **PDF Venue Map → stall-grid overlay/conversion — V2 (2026-06-12, Whitney, exploratory).** Let an admin **upload a PDF venue map** and either (a) overlay an interactive stall-pick layer on top of the PDF image, or (b) assist converting it into the native stall-grid system. Feasibility notes for v2 kickoff: true auto-conversion (OCR/vector parsing of a PDF into discrete numbered stalls) is hard and unreliable; the realistic MVP is **render the PDF to an image + let the admin drop/snap stall hotspots onto it** (manual overlay authoring), which reuses the existing map-builder grid as the data model and just swaps the blank canvas for the PDF backdrop. Pairs naturally with Facility Layout Templates (the overlay becomes a saveable venue template). Confirm scope + whether overlay-only is enough before building the conversion path.
