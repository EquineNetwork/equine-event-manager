...
- **Notifications / "Email all customers for this event" — V2 (2026-06-12, Whitney).** A first-class **Notifications** concept: admin composes an email (subject + body, branded template) and **sends it to every customer who has an order for a given event** (e.g. schedule change, weather, gate times, last-minute info). Builds on the existing **`EEM_Reservations_List_Page::render_email_customers_modal()`** (an Email Customers modal already exists on the Reservations list) — v2 formalizes it into a proper feature: dedicated compose UI, audience = distinct customers (by email) with an order on the event, optional audience filters (paid only / by type / by division), a **send log / history** (who/when/how many, reusing the activity-log pattern), and Emogrifier-inlined send via the existing transactional-email pipeline. Open at kickoff: per-recipient personalization tokens, throttling/batching for large sends, opt-out handling. Reuses the C11 confirmation-email render path.
