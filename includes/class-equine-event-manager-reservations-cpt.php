...
class EEM_Reservations_CPT {
	...
	public function sanitize_meta_submission( $source, $existing = array() ) {
		...
		$data = array(
			...
			'stall_tack_pricing_mode'         => in_array( isset( $source['stall_tack_pricing_mode'] ) ? sanitize_key( $source['stall_tack_pricing_mode'] ) : $existing['stall_tack_pricing_mode'], array( 'same', 'discounted', 'free' ), true ) ? ( isset( $source['stall_tack_pricing_mode'] ) ? sanitize_key( $source['stall_tack_pricing_mode'] ) : $existing['stall_tack_pricing_mode'] ) : 'same',
			'stall_tack_price'                => isset( $source['stall_tack_price'] ) ? $this->sanitize_money_value( $source['stall_tack_price'] ) : $existing['stall_tack_price'],
