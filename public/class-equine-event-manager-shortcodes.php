...
class EEM_Shortcodes {
	...
	/**
	 * v4 — Render the spreadsheet-driven facility map picker (customer side).
	 *
	 * The Stall Mapping equivalent of {@see self::render_stall_picker_grid()}: when
	 * a reservation has an imported map snapshot, customers pick stalls on the real
	 * facility map (full-screen modal, one tab per barn) instead of the flat chip
	 * grid. Selection drives the same checkout contract — it writes
	 * `preferred_stall_units[]` hidden inputs and sets `stall_qty` (= the number of
	 * stalls picked) so totals recompute exactly as the legacy picker does.
	 *
	 * @param array $snapshot       `_en_stall_map` snapshot (barns[].grid[][]).
	 * @param array $reserved       Reserved label lookup (label => true).
	 * @param array $blocked        Blocked label lookup (label => true).
	 * @param bool  $tack_enabled   Whether to offer the optional tack designation.
...
", (string) $message_text ) ) ) );
	 ...
	 * the items table is identical across surfaces. Each row carries
	 * section / desc / qty / units / rate / total (all display-formatted).
	 *
	 * @param array $order       Grouped order payload (EEM_Orders_Repository shape).
	 * @param bool  $include_fee When true, append the convenience-fee row (the
	 *                           email itemizes it inline). The receipt renders the
	 *                           fee in its dedicated totals block instead, so it
	 *                           passes false.
	 * @return array<int, array<string, string>> Ordered line-item rows.
	 */
	private function build_order_line_items( array $order, bool $include_fee = true ): array {
	...
	private function build_confirmation_email_html( array $order, bool $pdf_attached = false ): string {
		...
		$line_items = $this->build_order_line_items( $order, true );
	...
	/**
	 * Build the C12 mockup-faithful order receipt HTML.
	 *
	 * Shared by the PDF (Dompdf) and the hosted order page. Reuses the C11 line
	 * items + cancellation + special-requests data and adds the receipt-only
	 * Customer/Billing header, Reservation Summary cards, and the itemized totals
	...
	private function build_receipt_html( array $order, bool $for_pdf = false ): string {
		...
		// ── Itemized totals (per-section subtotal lines; sum == Subtotal) ──
		$totals = array();
		if ( (float) $stall_breakdown['base_subtotal'] > 0 ) {
		...
		if ( (float) $stall_breakdown['additional_shavings_subtotal'] > 0 ) {
			$totals[] = array( 'label' => __( 'Additional Shavings', 'equine-event-manager' ), 'value' => $money( $stall_breakdown['additional_shavings_subtotal'] ) );
		}
		if ( (float) $rv_base_subtotal > 0 ) {
		...
		if ( $stall_qty > 0 ) {
			$cards[] = array(
				...
				'rows'  => array(
					...
					array( 'label' => __( 'Req. Shavings', 'equine-event-manager' ), 'value' => (string) absint( $order['required_shavings_qty'] ) ),
				),
			);
		}
		if ( $rv_qty > 0 ) {
			...
			$cards[] = array(
				...
				'badge'       => sprintf( _n( '%d Reserved', '%d Reserved', $rv_qty, 'equine-event-manager' ), $rv_qty ),
				'badge_class' => 'rv',
				'rows'        => $rv_rows,
			);
		}
		if ( $group_rider_count > 0 ) {
			$cards[] = array(
				...
				'rows'        => array(
					...
					array( 'label' => __( 'Rider Names', 'equine-event-manager' ), 'value' => ! empty( $group_rider_names ) ? implode( ', ', $group_rider_names ) : '—' ),
				),
			);
		}

		// ── Header sub-line: dates · phone · email ──
		$sub_parts = array_filter( array(
			isset( $order['event_dates'] ) ? (string) $order['event_dates'] : '',
			$support_phone ? $this->format_phone_label( $support_phone ) : '',
			$support_email,
		) );
	...
	/**
	 * Compute the amount currently due on an order: component total plus C13.C
	 * custom line items minus the snapshotted discount. Mirrors the Collect
	 * Payment / Order Detail recomputed total so the charge matches the display.
	 *
	...
	/**
	 * AJAX (admin, C14) — create a Stripe PaymentIntent for collecting payment on
	 * an EXISTING order. Capability + nonce gated. Reuses the same Stripe REST
	 * primitive (`request_stripe_api`) as the customer invoice flow. No card data
	 * is handled here — the client confirms with Stripe Elements using the
	 * returned client_secret.
	 *
	 * @return void
	 */
	public function ajax_collect_payment_create_intent() {
	...
	/**
	 * AJAX (admin, C14) — verify a confirmed PaymentIntent and mark the order
	 * paid. Capability + nonce gated. Retrieves the intent server-side, asserts
	 * status=succeeded + order_key metadata match + amount match, then records the
	 * payment, captures card brand/last4 (CLEANUP #34, best-effort), and logs.
	 *
	 * @return void
	 */
	public function ajax_collect_payment_confirm() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'Permission denied.', 'equine-event-manager' ) ), 403 );
		}
		$order_key = isset( $_POST['order_key'] ) ? sanitize_text_field( wp_unslash( $_POST['order_key'] ) ) : '';
