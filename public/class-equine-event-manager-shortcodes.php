...
", (string) $message_text ) ) ) );
	...
	/**
	 * Build and write a lightweight PDF receipt attachment.
	 *
	 * @param array $order Order payload.
	 * @return string
	 */
	private function generate_receipt_pdf_attachment( $order ) {
		return '';
	}

	/**
	 * Build a basic text PDF for a reservation receipt.
	 *
	 * @param array $order Order payload.
	 * @return string
	 */
	private function build_simple_receipt_pdf( $order ) {
		$event_label      = ! empty( $order['reservation_title'] ) ? $order['reservation_title'] : $order['event_name'];
		$billing_details  = $this->get_billing_details_from_order_notes( $order['notes'] );
		$special_requests = $this->get_special_requests_from_order_notes( $order['notes'] );
		$stall_breakdown  = $this->get_order_stall_breakdown( $order );
		...
		$rv_base_subtotal = max( 0, (float) $order['rv_subtotal'] - $rv_addon_total );
		$lines = array(
			__( 'Equine Event Manager Reservation Receipt', 'equine-event-manager' ),
			'',
			sprintf( __( 'Event: %s', 'equine-event-manager' ), $event_label ),
			sprintf( __( 'Order Number: #%s', 'equine-event-manager' ), $order['order_number'] ),
			sprintf( __( 'Event Dates: %s', 'equine-event-manager' ), $order['event_dates'] ),
			'',
			sprintf( __( 'Customer: %s', 'equine-event-manager' ), $order['customer_name'] ),
			sprintf( __( 'Email: %s', 'equine-event-manager' ), $order['email'] ),
			sprintf( __( 'Phone: %s', 'equine-event-manager' ), $this->format_phone_label( $order['phone'] ) ),
			sprintf( __( 'Type: %s', 'equine-event-manager' ), $order['type'] ),
			sprintf( __( 'Status: %s', 'equine-event-manager' ), ucfirst( (string) $order['payment_status'] ) ),
			sprintf( __( 'Gateway: %s', 'equine-event-manager' ), ucfirst( str_replace( '_', ' ', (string) $order['payment_gateway'] ) ) ),
			sprintf( __( 'Created: %s', 'equine-event-manager' ), $order['created_at'] ),
			'',
		);

		if ( absint( $order['stall_quantity'] ) > 0 ) {
			$lines[] = __( 'Stall Reservation', 'equine-event-manager' );
			$lines[] = sprintf( __( '  Stay Type: %s', 'equine-event-manager' ), $this->format_stay_type_label( $order['stall_stay_type'] ) );
			$lines[] = sprintf( __( '  Arrival: %s', 'equine-event-manager' ), $this->format_reservation_date_label( $order['stall_arrival_date'] ) );
			$lines[] = sprintf( __( '  Departure: %s', 'equine-event-manager' ), $this->format_reservation_date_label( $order['stall_departure_date'] ) );
			$lines[] = sprintf( __( '  Stall Quantity: %d', 'equine-event-manager' ), absint( $order['stall_quantity'] ) );
			$lines[] = sprintf( __( '  Required Shavings: %d', 'equine-event-manager' ), absint( $order['required_shavings_qty'] ) );
			$lines[] = sprintf( __( '  Additional Shavings: %d', 'equine-event-manager' ), absint( $order['additional_shavings_qty'] ) );
			$lines[] = '';
		}

		if ( absint( $order['rv_quantity'] ) > 0 ) {
			$lines[] = __( 'RV Reservation', 'equine-event-manager' );
			$lines[] = sprintf( __( '  Stay Type: %s', 'equine-event-manager' ), $this->format_stay_type_label( $order['rv_stay_type'] ) );
			$lines[] = sprintf( __( '  Arrival: %s', 'equine-event-manager' ), $this->format_reservation_date_label( $order['rv_arrival_date'] ) );
			$lines[] = sprintf( __( '  Departure: %s', 'equine-event-manager' ), $this->format_reservation_date_label( $order['rv_departure_date'] ) );
			$lines[] = sprintf( __( '  RV Quantity: %d', 'equine-event-manager' ), absint( $order['rv_quantity'] ) );

			if ( ! empty( $order['rv_type'] ) ) {
				$lines[] = sprintf( __( '  RV Add-Ons: %s', 'equine-event-manager' ), $this->format_rv_type_label( $order['rv_type'] ) );
			}
		...
		if ( $group_rider_count > 0 ) {
			$lines[] = __( 'Group Reservation', 'equine-event-manager' );
			$lines[] = sprintf( __( '  Riders in Group: %d', 'equine-event-manager' ), $group_rider_count );

			if ( ! empty( $group_rider_names ) ) {
				$lines[] = sprintf( __( '  Rider Names: %s', 'equine-event-manager' ), implode( ', ', $group_rider_names ) );
			}

			$lines[] = '';
		}

		$lines[] = __( 'Payment Breakdown', 'equine-event-manager' );

		if ( (float) $stall_breakdown['base_subtotal'] > 0 ) {
			$lines[] = sprintf( __( '  Stall Subtotal: $%s', 'equine-event-manager' ), number_format_i18n( (float) $stall_breakdown['base_subtotal'], 2 ) );
		}

		if ( (float) $stall_breakdown['required_shavings_subtotal'] > 0 ) {
			$lines[] = sprintf( __( '  Required Shavings: $%s', 'equine-event-manager' ), number_format_i18n( (float) $stall_breakdown['required_shavings_subtotal'], 2 ) );
		}

		if ( (float) $stall_breakdown['additional_shavings_subtotal'] > 0 ) {
			$lines[] = sprintf( __( '  Additional Shavings: $%s', 'equine-event-manager' ), number_format_i18n( (float) $stall_breakdown['additional_shavings_subtotal'], 2 ) );
		}

		if ( (float) $rv_base_subtotal > 0 ) {
			$lines[] = sprintf( __( '  RV Subtotal: $%s', 'equine-event-manager' ), number_format_i18n( (float) $rv_base_subtotal, 2 ) );
		}
		...
		foreach ( $rv_addon_breakdown as $addon_row ) {
			...
			$lines[] = sprintf( __( '  %1$s Add-On: $%2$s', 'equine-event-manager' ), $addon_row['label'], number_format_i18n( (float) $addon_row['subtotal'], 2 ) );
		}
		...
		foreach ( $group_charges as $group_charge ) {
			...
			$lines[] = sprintf( __( '  %1$s: $%2$s', 'equine-event-manager' ), $group_charge['label'], number_format_i18n( (float) $group_charge['subtotal'], 2 ) );
		}

		if ( (float) $order['fees'] > 0 ) {
			$lines[] = sprintf( __( '  Non-Refundable Convenience Fee: $%s', 'equine-event-manager' ), number_format_i18n( (float) $order['fees'], 2 ) );
		}

		$lines[] = sprintf( __( '  Total Amount Paid: $%s', 'equine-event-manager' ), number_format_i18n( (float) $order['total'], 2 ) );
		$lines[] = '';

		if ( '' !== $billing_details ) {
			$lines[] = __( 'Billing Details', 'equine-event-manager' );

			foreach ( preg_split( "/\r\n|\r|\n/", $billing_details ) as $billing_line ) {
				$lines[] = '  ' . $billing_line;
			}
		...
		if ( '' !== $special_requests ) {
			$lines[] = __( 'Special Requests', 'equine-event-manager' );

			foreach ( preg_split( "/\r\n|\r|\n/", $special_requests ) as $request_line ) {
				$lines[] = '  ' . $request_line;
			}
		}

		return $this->render_simple_pdf_document( $lines );
	}

	/**
	 * Render a simple single-page text PDF document.
	 *
	 * @param array $lines Text lines.
	 * @return string
	 */
	private function render_simple_pdf_document( $lines ) {
		$wrapped_lines = array();
		...
		$wrapped_lines = array_slice( $wrapped_lines, 0, 52 );
		$content = "BT\n/F1 12 Tf\n14 TL\n50 770 Td\n";

		foreach ( $wrapped_lines as $index => $line ) {
			if ( 0 !== $index ) {
				$content .= "T*\n";
			}
	...
	/**
	 * Escape text for simple PDF output.
	 *
