...
**Last updated:** 2026-05-29 — **C8.H mockup** — Customer Selection toggle deleted entirely from both Stall and RV mapped-content wrappers. Inventory Mode now determines customer experience absolutely: Bulk = quantity picker at checkout (admin assigns stalls/lots post-purchase on the Stall & RV Charts page); Mapped = customer selects specific stalls/lots from the layout at checkout (Interactive Map ships at C10+; graceful degradation handled at C10 render time). toggleCustomerSelection() deleted. updateRvInventoryDisplay() simplified — selBtn/selMode branch removed, always sums zone-qty inputs in Mapped mode. .badge-coming-soon CSS deleted. Inventory Mode hint text updated for all four cases. Commit 59c0031. 2486 lines.
