...
| 6 | `.stall-row-layout` CSS class — REAL runtime collision, not preemptive | `stall_chart_detail.html` | Original row-builder source uses `.stall-row`. The collision was real: the row-builder's `.stall-row{display:flex;flex-wrap:wrap}` and `.stall-row.back-to-back{flex-direction:column}` would have converted `<tr class="stall-row">` occupancy-table rows into flex containers, breaking the chart layout. Scoping via `.row-builder .stall-row` was **insufficient** — chart preview modals generate `<div class="stall-row">` inside `#chart-preview-modal .chart-modal-body`, which is outside `.row-builder`, requiring a second selector. Rename to `.stall-row-layout` covers all generated HTML in one declaration. **Port action:** do NOT carry `.stall-row-layout` into production. Use `.eem-stall-row-layout` or another canonical `.eem-*` name, or reclaim `.stall-row` with explicit scoping on both the builder and modal containers. |
