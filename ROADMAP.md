...
> **🔁 v1 RE-SCOPE (Whitney 2026-06-29):** moved to v2 — Admin unsubscribe/resubscribe toggle, "X days" countdown chip, TEC event-list template. **Group Names verify (was its own item)** → I verify it myself as one combination inside the FULL VERIFICATION PASS. **PAYMENT-AUDIT-2 + Full checkout sweep + remaining CHECKOUT-VALIDATION-AUDIT config gates** → all folded into the single master item below.

- [ ] **🚨🚨 V1 FULL VERIFICATION PASS — seed every data combination, run a REAL checkout for each, verify EVERY surface calculates + displays correctly (Whitney 2026-06-29, TOP v1 PRIORITY, TRUST-CRITICAL).**
  **PROGRESS (2026-07-01, → living doc `docs/V1-VERIFICATION-COVERAGE.md`):** coverage audit done — the charge-side (calculator, write path, refunds, discounts, custom items) is strongly covered behaviorally; gaps are specific. Closed the **stay-type gap** (`stay-type-matrix-allsurfaces-smoke`, 2.7.734): weekly/weekend (stall+RV) now priced with non-zero rates + reconciled charge→stored→email; confirmed weekly/weekend bill ONCE not ×nights (no bug). Also: the suite runner now counts the 12 `PASS=/FAIL=` money-math smokes + hard-fails on regression (was a blind spot); a PHP 8 unit-pool notice guarded. **REMAINING:** (1) 🔴 customer JS Order Summary vs charge → the REAL-BROWSER session with Whitney (only surface no in-process smoke reaches); (2) package/surcharge orders at OD/PDF render; (3) hosted-page reconcile; (4) Reports on a composite order. **Test-isolation NOTE (feeds #55):** `order-edit-append-engine-smoke` picks "the most recent order" as its target and asserts a clean +delta — it breaks if that target carries a convenience fee (add_component_quantity recomputes the row fee from the passed pricing). Harden it to seed its own fee-free target (deterministic) rather than picking from the live pool. Whitney: "a FULL data seeding / checkout / test of every single process and combination and surface to make sure everything is working and calculating one more time." This is the launch-readiness pass and supersedes PAYMENT-AUDIT-2, the Full checkout sweep, Group-Names verify, and the remaining config-conditional checkout-validation gates. **Combinations to seed + check out (each as a real order):** stall (numbered + bulk; nightly / weekend / weekly / packages / both; early-bird vs regular; with + without tack; full map pick + auto-assign), RV (same matrix; map + quantity; with RV-lot surcharge/zones), required shavings + additional shavings (per-type), general add-ons, group reservation (grounds fee + deposit; self-is-rider on/off; N riders), pre-entries (+ division spots), convenience fee ($ + % ; card vs cash/check skip), tax, discounts ($ + %), custom line items, required documents. **Surfaces to reconcile per order (must all match the charge to the penny):** customer checkout Order Summary (live JS), the actual Stripe/Auth.net charge, admin Order Detail, PDF receipt, hosted order page, confirmation email, Reports, Stall & RV Charts assignment. **Method:** real browser checkouts + computed-value verification (NOT source-presence smokes). Build/extend the seeding harness so combinations are repeatable; produce a findings doc with a pass/fail matrix. Needs configured fixtures for the config-conditional gates (required docs, pre-entries, RV-lot, inventory caps) since only the corrupted 5990 has some today.

- [ ] **🚨 CHECKOUT-VALIDATION-AUDIT — full end-to-end audit of every checkout gate / required-selection rule (Whitney 2026-06-29, HIGH TRUST-CRITICAL). [server-gate sweep DONE 2.7.708; remaining config-conditional gates folded into the FULL VERIFICATION PASS above]** Triggered by the all-or-nothing stall-pick bug: with the map ON, the form let a customer set quantity = 3, pick only 2 stalls on the map, and still check out (no prompt, no block). FIXED in 2.7.704 (server gate in `validate_submission` + live red warning + capture-phase submit guard `bindStallPickGuard` + `syncStallPickWarning`; behavioral guard `tests/smoke/stall-pick-all-or-nothing-smoke.php` 3/0). But Whitney (rightly) wants a SYSTEMATIC sweep for the whole class of "you shouldn't be able to check out unless X" gaps — these keep slipping through. **Scope — exhaustively verify, via REAL browser checkouts, that checkout is blocked (with a clear prompt) until every required condition is met:** (a) **stall map** — picks must equal qty OR be zero (auto-assign); partial blocked [DONE, but re-verify the row-picker grid variant too, not just the v4 map]; (b) **RV map** — same all-or-nothing rule for `preferred_rv_lots` vs `rv_qty` (LIKELY HAS THE SAME BUG — audit + fix); (c) at-least-one-item gate; (d) required documents; (e) group riders (count ≥ 1, every rider name filled); (f) stay dates present + valid for each enabled section (stall/RV); (g) billing details complete; (h) phone/email format; (i) per-customer pre-entry caps + division spots-left; (j) tack stall must be one of the picks; (k) qty within remaining inventory. For EACH: confirm the client shows a prompt AND the server rejects (no charge), and that the happy path still completes. **Method:** behavioral (drive the real form), not source-presence smokes. Produce a findings doc + a behavioral smoke per gate. Pairs with PAYMENT-AUDIT-2 (same "first audit missed it" root concern, different surface — validation vs money math). **PROGRESS (2.7.708):** behavioral sweep of the server validator landed — `tests/smoke/checkout-validation-gates-smoke.php` (14/0) drives the real `validate_submission()` for contact/email/phone/billing/token/at-least-one-item/group-count/group-names/stall-partial-pick/stall-over-pick/stall-dates. Found + fixed a real gap: **over-pick (more picks than qty) was not server-validated on MAP reservations** (the units>qty check was gated on `stall_chart_enabled`, which is 0 for v4-map reservations) — added exact_map over-pick gates for stall + RV. Findings doc: `docs/CHECKOUT-VALIDATION-AUDIT.md`. **REMAINING:** behaviorally exercise the config-conditional gates (required documents, pre-entry caps + division spots, sold-out/inventory/max-per-customer, RV lot selection, RV-map partial/over-pick, package-date matching) on a fixture that enables them — all are code-reviewed present but not yet seeded.

- [ ] **🚨 PAYMENT-AUDIT-2 — full re-audit of every money calculation (Whitney 2026-06-29, HIGH TRUST-CRITICAL).** Triggered because the group-reservation work surfaced multiple money bugs that the first audit / smokes missed: (a) the customer form read section toggles + group names from stale post-meta instead of the config table (sections/dropdowns silently absent at checkout); (b) the group section pre-charged **$200 on load** with Number of Riders = 0, caused by TWO separate `Math.max(1, …)` clamps on `group_rider_count` (rider-row renderer at ~13068 AND the price recalc at ~13770) — the first fix only patched one. Whitney does NOT trust the calculations. **Scope:** re-verify EVERY money surface end-to-end against an ACTUAL browser checkout (not just source/smoke): stall (numbered + bulk, nightly/weekend/weekly/packages, early-bird), RV (same), required + additional shavings, general add-ons, group rider grounds fee + deposit (0 riders = $0, N riders = N×fee, toggle off = $0), pre-entries, convenience fee (global, $/%, card-only skip on cash/check), tax, and the grand total — reconciled identically across customer checkout JS, admin Order Detail, PDF receipt, and confirmation email. **Method requirement:** behavioral checks via real checkouts + computed-value verification, NOT source-presence smokes (those are exactly what missed the clamp). Cross-check the JS recalc against `calculate_submission_totals()` for every combination. Audit the client/server parity especially: any `Math.max`/clamp/default on a quantity or rider/unit count, and any field the form reads from post-meta that the editor writes to the config table. Produce a findings doc.

- [ ] **💵 V1 — Refund admin-added custom charges (Whitney 2026-07-01).** An admin can ADD a custom line item (late fee, damage charge, transferred credit) to an order and charge it, but the refund system can only place refunds against reservation *components* — a custom charge has no component to land against, so it CANNOT be refunded (fails SAFE: errors, never over-pays; money stranded). **FULL DESIGN + SCOPING DONE → `docs/REFUND-CUSTOM-CHARGES-DESIGN.md`.** It is NOT a quick edit — the pieces are interdependent (can't relax the ceiling without working placement), it needs a small schema addition to track that a custom charge was refunded, and its core new primitive (gateway refund against the Collect-Payment charge, not a component) can only be validated in **Stripe test mode**. **DECISION (Whitney 2026-07-01): build it as ONE UNIT in the Stripe test session** (paired with the browser pass) — fresh + sharp + live-validated, not half-tested. Design doc is the running start.

- [ ] **💵 V1 — Discount behavior on order edit (Whitney 2026-07-01, from the money re-audit).** A percentage discount is frozen to a dollar amount at set-time. If an order is edited SHORTER (Edit Dates) below that frozen amount, the discount no longer tracks the new subtotal. **Current shipped behavior (safe stopgap, 2.7.732):** the frozen dollar amount stays and the composed grand total floors at $0 (so it can't go negative or inflate "Refund Owed" — #23). **Whitney: "build for sure" — v1. OPEN sub-decision to confirm at build time:** should a % discount RE-RESOLVE against the edited subtotal (50%-off recomputes to 50% of the smaller amount) or stay FROZEN (contract integrity — the exact deal the customer agreed to)? My recommendation: **re-resolve for a percentage** discount (more intuitive), keep **frozen for a fixed-dollar** discount. Whichever is chosen, recompute on every edit path (Edit Dates + Add Items) and reconcile all downstream surfaces.

- [x] **🐞 CHECKOUT BUG — Stripe "payment amount did not match this reservation total" + form wipes on error (Whitney 2026-06-29, BLOCKING) — ✅ FIXED + LIVE-VERIFIED (2.7.702).** ROOT CAUSE: `ajax_create_stripe_payment_intent` built the submission WITHOUT first setting `$this->active_reservation_id`. The validator chain `sanitize_submission()` → `sanitize_preferred_stall_units()` → `get_stall_map_unit_labels()` reads that property to merge the stall-MAP cell labels (e.g. "274", "275") into the allowed unit pool. Unset (0), the map labels were absent → every map-picked stall was intersected away → the designated tack stall was discarded → the PaymentIntent total was computed WITHOUT the tack-shavings exclusion (e.g. $332.80) while the final submit (which DOES set the context) computed WITH it ($312.00) → amount-match at `shortcodes.php` rejected the charge. FIX: set `$this->active_reservation_id = (int) $reservation_id;` in the intent handler before `sanitize_submission()` (one line; mirrors `handle_reservation_submission`). Also landed: intent path now runs `resolve_rv_tier_submission`/`resolve_stall_tier_submission` for parity, and the idempotency key now includes the amount (a changed cart on the same page load creates a fresh intent instead of returning a stale one). VERIFIED: drove 3 real map-picked tack checkouts end-to-end via Chrome — all created orders (#91648, #91649) at $312.00 with the charge reconciling exactly, no mismatch error. Behavioral guard added: `tests/smoke/stripe-intent-map-pick-parity-smoke.php` (8/0 — reproduces the drop without the context, confirms survival with it). FORM-WIPE bug also fixed: restore no longer marks itself done on an empty/early read (so a later pass can still repopulate after a checkout error), plus a delayed re-init and tack-consistency normalization at every serialization point. Diagnostics removed.

- [ ] **→ MOVED TO v2 (Whitney 2026-06-29).** Admin manual unsubscribe / resubscribe toggle on the customer profile. The customer profile now SHOWS email opt-in status (Subscribed/Unsubscribed). Add an admin control to TOGGLE it — so when a customer calls and asks to be removed (or re-added) to bulk update emails, the admin can do it from the profile. Wire to `EEM_Email_Optout::record_optout()` / removal from the `eem_email_optouts` option, with a nonce + capability check, and an activity-log entry. Small, builds directly on the opt-out system from #22.

- [ ] **→ FOLDED INTO V1 FULL VERIFICATION PASS (Whitney 2026-06-29 — "you can test yourself").** Group Names feature — verify when groups are actually in use (not yet). Shipped 2.7.650 + branch follow-ups. Verify: (1) admin adds names in the editor Group Names table; (2) customer event page shows the strict-list Group dropdown; (3) assign/change/remove group from the map popover; (4) sidebar Groups filter (only when groups enabled); (5) group shows on order detail; (6) **Grounds Fee + Rider Deposit charges show on the customer Order Summary AND admin Order Detail** with correct per-rider totals. Editor-cleanup commit `1bc0432` on the branch is NOT yet merged/deployed — bump + merge when ready to verify.



3. [ ] **→ FOLDED INTO V1 FULL VERIFICATION PASS (Whitney 2026-06-29).** Full end-to-end customer checkout sweep — run a real checkout on the NTR 6519 fixture page. Also the recommended way to seed test data (real checkout writes correct `reservation_id` + notes tag + config-based pricing).

- [ ] **#41 Collapse the 42 one-time migrations into a baseline — SCOPED 2026-07-01, NOT a file-delete.** Finding: the activator's `dbDelta` `CREATE TABLE` definitions are OUT OF SYNC with the live schema — `eem_stall_reservations` dbDelta defines 33 cols but the live table has 36; `selected_package_ids`, `effective_start_date`, `effective_end_date` exist ONLY because migrations ALTERed them in, NOT in the base definition. So the schema migrations are **load-bearing for fresh installs** — deleting them without folding those columns into dbDelta would break a fresh install (missing columns) AND dbDelta upgrades already don't add them. **Safe plan (needs a scratch DB — do NOT test dbDelta against the live tables, its ALTERs are side-effectful):** (1) sync EVERY table's dbDelta definition to the exact live schema (`SHOW CREATE TABLE` per table: stall/rv/config/adjustments/payments/event-defaults/activity-log/…); (2) on a scratch prefix, run the activator create → assert the tables match live exactly; (3) run dbDelta against a COPY of the live tables → assert 0 ALTERs (proves existing sites no-op); (4) only THEN remove the migration files + `run_one_time_migrations`. Execute sharp with the scratch-DB harness, like #5. Fixing the dbDelta drift (step 1) is itself a real latent-bug fix worth doing even before the collapse.
...
14. [ ] **→ MOVED TO v2 (Whitney 2026-06-29).** Remove "X days" countdown chip on the event-list flyer card (needs Whitney's mockup).

20. [ ] **→ MOVED TO v2 (Whitney 2026-06-29).** TEC event list template — frontend event-list display for TEC-sourced events.
