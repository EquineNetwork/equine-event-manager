...
- [ ] **💵 V1 — Refund admin-added custom charges (Whitney 2026-07-01, from the money re-audit).** Today an admin can ADD a custom line item (late fee, damage charge, transferred credit, etc.) to an order and charge it, but the refund system can only place refunds against the original reservation *components* — a custom charge has no component to land against, so it CANNOT be refunded. It fails SAFE (the engine errors rather than over-paying), but the money is stranded once collected. **Whitney: "we want to build for sure" — v1.** Build: teach the refund placement (`EEM_Refund_Engine` — the placement loop reads only `$order['components']` today) to also place against order-level adjustments (custom items in `EEM_Order_Adjustments_Repo`). The refund *ceiling* is already correct (`get_order_refundable_ceiling` caps at gross-collected − already-refunded, #20); the gap is purely placement. Add a behavioral smoke: add a custom item, charge it, refund it, assert the ledger + balance + all surfaces reconcile.
