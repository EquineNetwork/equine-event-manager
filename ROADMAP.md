...
A3. [ ] **Bulk-notification unsubscribe** — DECISION (Whitney 2026-06-27): **build for v1.** Add a per-recipient opt-out: unsubscribe link (HMAC-signed, WP-salt keyed) in bulk emails (Notifications page + Email Customers), a public handler that records the opt-out, and a skip-check before non-transactional sends. Transactional sends (confirmation, refund, payment, invoice, cancellation) always send regardless. **Status: approved for v1, NOT started.**
...
A12. [ ] **Payment-gateway key at-rest encryption** — DECISION (Whitney 2026-06-27): **harden for v1.** Encrypt the secret keys (Stripe secret/webhook, Auth.net transaction key) in `equine_event_manager_payment_settings` at rest, keyed off WP salts (`SECURE_AUTH_KEY` etc.), with a one-time migration to encrypt existing stored keys. **Sequencing:** isolated to the settings layer (`class-eem-settings-repo.php` read + `class-eem-settings-page.php` save) — the payment-dispatch fixes the desktop is doing live in `shortcodes.php` / `refund-engine.php` don't touch these files, so low collision. BUT security-critical: if decryption fails, live payments break. **MUST be live-verified by Whitney before merge** (process a real test charge after the migration runs). Keep all getter return contracts identical (callers still receive plaintext) so payment dispatch is untouched. **Status: approved for v1, NOT started. Sequenced last; coordinate the merge with the desktop payment work.**

A13. [ ] **Auto payment-reminder for unpaid orders** — DECISION (Whitney 2026-06-27): **wire an auto-trigger.** A daily WP-cron finds unpaid orders past a configurable age and sends the existing `PAYMENT_REMINDER` template, with dedupe so a given order isn't reminded repeatedly. Reads order payment_status + sends email only — no payment-dispatch changes, so safe alongside the desktop payment work. **Status: approved for v1, NOT started.**
