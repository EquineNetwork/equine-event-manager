...
- [ ] **🐞 CHECKOUT BUG — Stripe "payment amount did not match this reservation total" + form wipes on error (Whitney 2026-06-29, BLOCKING).** Hit on a real 18375 Stripe checkout: the card confirmed (succeeded) but the final-submit amount-match at `shortcodes.php:4794` rejected it, AND the whole form cleared so the customer would re-enter everything. Root-cause hunt in progress: the PaymentIntent total (`ajax_create_stripe_payment_intent`) diverges from the final-submit recompute (`process_payment_submission`→`calculate_submission_totals`). Already fixed one structural gap — the intent path was missing the `resolve_rv_tier_submission`/`resolve_stall_tier_submission` the submit path runs (no-op for map-picked stalls, so not 18375's cause). Temp diagnostic logging added at both points (gated WP_DEBUG) to capture the exact divergence on retry — **REMOVE the diagnostics + commit the real fix once found.** Second bug: form-state restore must repopulate on a checkout error (currently wipes — major data-loss UX bug). Both are part of PAYMENT-AUDIT-2 but block checkout NOW.
