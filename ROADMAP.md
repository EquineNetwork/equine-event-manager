...
- [ ] **Stall & RV Charts blue metrics bar** (was #26). The chart detail page's KPI bar is restyled into the navy gradient **Daily-Movement hero** look and enriched from 4 → 6 metrics: **Total / In Use / Available** for both Stalls and RV Lots ("In Use" = peak concurrent usage from the grid's `peak_*_used`; "Available" = total − in use). Markup keeps the `data-inv-section` hooks so the Stalls/RV/All filter still shows the right cells. Smoke: `tests/smoke/stall-chart-metrics-bar-smoke.php` — pure file-scan, **self-verified this session 15/15**. **Verify visually on a chart-enabled reservation (NTR 6519): blue band, correct numbers, filter still works.**
...
26. ➡️ _Moved to **For Review** (done this session — KPI bar restyled to the blue Daily-Movement hero + In Use/Available metrics)._
