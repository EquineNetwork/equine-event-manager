...
- **Status:** ✅ RESOLVED in C16 (v2.7.127) — **DEAD code, deleted.** The v2.7.123 "NOT dead, ~30 live callers" reclassification was itself wrong: those ~30 callers are the `render_*_meta_box()` methods, which are **themselves dead** — there is NO `add_meta_box()` registration for `en_reservation` (the bootstrap comment in `class-equine-event-manager.php` confirms the wiring is "intentionally absent"), and the CPT edit screen redirects to the custom `equine-event-manager-reservation-editor` admin page that renders 100% via `templates/admin/reservation-editor/` partials. Browser-verified: the live editor DOM has **0** legacy `<tr>` field rows and **70** canonical `.eem-field-row`; partials never call the helpers (only one stale comment in `_section-agreement.php`). So the whole chain — 18 dead `render_*_meta_box()` methods + 9 `render_editor_*_row()` wrappers + 8 private `render_*_row()` helpers — was orphaned. Removed via call-graph-verified deletion map (sub-agent confirmed the dead set is **closed**: no live method calls in; live `render_tec_event_meta_box` / `render_native_event_meta_box` and all `sanitize_*` / `expand_chart_units` / `sanitize_chart_*` helpers preserved). **1,283 LOC removed** (496 from the admin editor class, 787 from the CPT class). Verified post-delete: both files lint clean, 0 remaining references to any deleted method, editor renders + AJAX-saves identically. **Micro-follow-up swept in v2.7.128:** the three now-orphaned helpers (`get_overview_actions`, `render_chart_blocked_units_field`, `parse_time_value` — all re-confirmed 0 callers) deleted (−112 LOC), and the two stale comments (`_section-agreement.php` + `assets/js/admin.js`) corrected to reflect the partial-based render. Both files lint clean; zero remaining references.
...
- **Status:** ✅ RESOLVED (v2.7.135). New `EEM_Refund_Engine` (`includes/class-eem-refund-engine.php`) owns the refund kernel: `process_amount_refund` + `refund_order_component` + `refund_with_stripe` + `refund_with_authorize_net` + `get_component_refunded_amount` + `get_component_remaining_refundable_amount` + `persist_component_refund` — moved **byte-for-byte** (script-extracted, zero transcription risk). The 4 stateless EEM_Admin helpers the kernel needs (`get_payment_settings`, `get_order_note_value`, `get_component_refunded_item_ids`, `upsert_order_note_line` — 6–13 non-refund callers each) stay on EEM_Admin, widened to `public`, reached via an injected `$admin` reference; the engine news up its own `EEM_Orders_Repository`. **Discovered during implementation:** the 7 methods are also called by the itemized-refund (`build_*`) and bulk-refund paths, not just single-refund — so instead of relocating-and-rewiring every caller, EEM_Admin keeps **thin 1-line delegators** (`return $this->refund_engine()->method(...)`), so the AJAX single-refund, itemized, and bulk paths all call the same method names unchanged. Behavior-preserving. Verified live: delegator↔engine parity (remaining-refundable math identical), gateway-dispatch routing (unsupported → WP_Error), Stripe path reaches the live HTTP call (proves `get_payment_settings` wiring) and returns a graceful error for a bogus transaction, and the visibility split is correct (4 shared helpers + the `process_amount_refund` AJAX entry public; refund math/dispatch private). NOTE: a real settled-gateway refund (live Stripe/Auth.net transaction) was not exercised — no test transaction available — but the gateway HTTP code moved verbatim and the Stripe dispatch path was confirmed reachable. Future C6.C `process_bulk(...)` can move into the engine the same way.
...
- **Status:** ✅ RESOLVED (v2.7.134). `EEM_Reservation_Editor_Page::ajax_save()` snapshots `get_meta_values()` either side of `save_meta()` and calls `EEM_Reservations_CPT::log_reservation_edit_diff()`, which `compute_scalar_meta_diff()` (scalar fields only — nested arrays/maps skipped; noise/derived/cache keys excluded via `DIFF_NOISE_FIELDS`) and, when something changed, writes a `reservation_edited` Activity Log entry: `title` "Reservation edited by {admin}", `meta` "Label: old → new · …" (humanized via `DIFF_FIELD_LABELS`, capped at 8 + overflow count, em-dash for empty values), plus a structured `changes` payload. Renders amber ('edit' variant) in the C2 partial. **Display surface (per Whitney):** Order Detail's `render_activity_log()` now merges reservation-scoped entries (`get_for_reservation`) with the order-key entries — de-duped by id, re-sorted newest-first — so a reservation's config edits show as context on its orders. Verified live: diff computes correctly + filters noise, entry round-trips through `get_for_reservation`, and renders with the edit variant + arrow.
...
- **Status:** ✅ RESOLVED (v2.7.132). `EEM_Reservation_Source_Resolver::on_native_event_saved()` / `on_tec_event_saved()` registered on `save_post_en_event` / `save_post_tribe_events` at priority 40 (after the event's own meta save). Both delegate to `sync_linked_reservations( $event_id, $source )`, which matches reservations by the `_en_event_id` + `_en_event_source` meta pair and re-runs `cache_source_event_start_date()` on each (live re-resolve → rewrite `_en_source_event_start_date`). Revision/autosave guarded. Verified live: corrupting reservation 5990's cache then firing the sync heals it back to the live-resolved source-event start_date; both hooks confirmed registered. (Feed source still has no save hook — documented as the remaining gap, needs a cron-on-transient-expiry; native + TEC, the editable WP-post sources, are covered.)
...
- **C16 ACTUAL outcome (v2.7.123) — form-control cartel STRIPPED, button cartel DEFERRED (corrected analysis):**
  ...
  - **Button cartel: generic blocks STRIPPED (v2.7.126), corrected from the earlier "partly load-bearing" read.** The 3 generic blocks (`.button:not(.button-link)` / `.button.button-primary` / `:hover` incl. the `body.eem-shell-page .wp-core-ui .button` selector + post-type mirrors) were removed after a browser audit proved the live blast radius is **WP chrome only**: every ported shell page (reservation-editor, create-order, stall-charts, dashboard) has **ZERO** plugin-owned WP-native `.button` elements — the plugin uses `.eem-btn*` exclusively. The one live consumer was the WP Media Library modal, whose buttons were being wrongly force-tinted plugin-pink; post-strip they correctly render WP-native (verified: "Use this file" now shows the WP Modern color-scheme dark primary, "Select Files" the WP secondary). Nothing plugin-owned depended on the blocks, so they were stripped, not migrated to admin.css (adding unused `.button` rules would be cruft). The **page-component** button blocks (`.eem-stall-chart-actions .button`, `--orders` toolbar buttons, `--stall-chart`/`--reports`/`--invoicing`/`--overview` `.button-primary`, etc.) are PRESERVED — page-specific, possibly still styling WP buttons on detail views not audited here. Future optional cleanup: probe each of those pages for native `.button` usage and strip the now-dead page-scoped blocks too.
...
- **Status:** ✅ RESOLVED in C16 (v2.7.126) — browser-verified the actual rendered search pair on the Reservations list. The `.eem-search-input` + `.eem-toolbar-btn.eem-search-btn` pair already renders the intended attached treatment: input left corners 3px / right corners square, button left corners square + `border-left: 0` / right corners 3px, both 29px tall, and the boxes touch (measured gap = -1px). No double border, no visible seam. The earlier "all four corners rounded / visible seam" symptom is gone (most likely fixed incidentally by the C7.X.13 form-control prefix work + the C16 form-control cartel strip). One cosmetic residue: a WP-core `margin: 0 1px` leaks onto the input, but since the boxes still touch it produces no visible gap. The search button is an `.eem-*` component (not WP-native `.button`), so it was never the generic button cartel; the #1 strip and #13 are independent — both now correct.
...
- **Status:** ✅ RESOLVED in C16 (v2.7.125) — Whitney chose "remove the Edit option, don't ship a placeholder." Field-level bulk edit was a sizable feature with ambiguous scope (which reservation fields are bulk-editable?), and per-row Edit already covers the need. Note: the custom Path-B toolbar dropdown never actually exposed an `Edit` option — only status moves (Publish / Switch to Draft / Move to Trash, plus Restore / Delete Permanently on the Trash tab). So the `case 'edit'` branch + `bulk_edit_unsupported` notice were dead code. Removed both; any unrecognized action now falls through to the existing `bulk_no_action` notice. No UI change (dropdown was already Edit-free).
...
- **Status:** ✅ RESOLVED (v2.7.131). Added a denormalized `reservation_id bigint unsigned NOT NULL DEFAULT 0` column + `KEY reservation_id` to BOTH order tables via dbDelta (the column did NOT pre-exist — the C12 `reservation_id` was on a different table). Checkout inserts now populate it; `eem-mig-009` backfills legacy rows from the `Reservation setup ID: N` notes line. `get_orders_count_for_reservation()` switched to indexed `WHERE reservation_id = %d` (also fixes the `notes LIKE '%…: 5%'` prefix-match bug where id 5 matched 50/51/…). The `orderby=orders` path replaced its fetch-up-to-500 + PHP usort + hand-paginate with a `posts_clauses` filter that LEFT JOINs a per-reservation orders-count derived table and ORDER BYs the count — WP_Query paginates normally with correct found_posts, **500-row cap removed**. Verified live: column + index present, 9/9 legacy rows backfilled, count parity (indexed=9 == notes-LIKE=9), and the SQL-JOIN sort renders + paginates correctly in the browser both directions.
...
- **Status:** ✅ RESOLVED (v2.7.130). The `tax` + `tax_rate` columns were added to both order tables in C12 and the orders-repository aggregation adds tax into the grouped order total + exposes it as the receipt's Sales Tax line (so the core "total excludes tax" bug was already fixed). C12 allocated tax all-on-the-stall-row (RV row got 0 when a stall order existed) — aggregate-correct but it gave split stall+RV orders a $0-tax RV component row. This pass replaces that with **proportional allocation** by taxable subtotal in the checkout insert (`shortcodes.php`): stall row takes the rounded share, RV row takes the exact remainder, so the two always sum to `$totals['tax']` to the cent. Single-component orders are unchanged (all tax on the one component). Row `total` stays subtotal+fee (refund/component logic untouched). Verified: split math passes the exact-sum + proportionality invariants across even/uneven/rounding-edge/single-component/zero-base cases. No migration needed — column pre-existed; only new checkouts get the proportional split.
