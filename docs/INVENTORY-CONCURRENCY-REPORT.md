...
| LOW-6 | **CLOSED — not a bug (2026-06-20)** | `add_component_quantity` adds the engine-priced delta onto the stored subtotal. On re-exam this is *correct + intentional*: it **preserves the customer's originally-locked rate** (early-bird / package) and bills only newly-added units at the current rate. "Recompute as current rate × qty × nights" would be a *real* bug (wipes original EB/package pricing on a later add). The #90801 mismatch was a **seed/test artifact**, not the normal flow; with seed-only data there's nothing to reconcile. No code change. Product decision: no manual/custom rates — discounts go through the order-screen Discount feature. | `orders-repository.php:651` | — |
