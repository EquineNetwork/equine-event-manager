...
**Last updated:** 2026-06-02 — **C13.B.2.a** — Server-side embed of `[en_reservation]` form on Create Order page. When `?reservation_id=N` is set: (1) outer workspace is `<div>` (not `<form>`) to prevent invalid nested forms; (2) `render_reservation_card_picked()` shows the blue linked-reservation pill (info-bg + Change link back to base URL); (3) `render_embedded_sections()` calls `do_shortcode('[en_reservation id=N]')` inside `.eem-co-form-embed` — pricing JS (qty steppers, date pickers, `updateReservationTotals`) embedded inline because `is_admin()` triggers `render_form_styles()` immediately; (4) public.css enqueued via `admin_enqueue_scripts` (the footer hook guards against `is_admin()`); (5) admin.css `.eem-co-form-embed` scope rules hide contact / stay-details / pre-entries / special-requests / payment / summary-rail, strip wrapper chrome. `window.eemCreateOrder.reservationId` localized for B.2.b/c. Group section appears in embed block (before Custom Items) — monolith tradeoff, accepted for B.2.a. Smoke c13b2a 37/37. Version 2.7.3. Commit fbd0126.

**Next: C13.B.2.b** — mirror the embedded form's running total into the Create Order rail summary. Read `data-eem-total` values from the hidden `.eem-reservation-workspace__rail` and render matching `.eem-co-summary-line` rows + total in our rail on every pricing-engine event.

**Last updated (prev):** 2026-05-29 — **C8.I mockup** — Header restructured across edit_reservation_page.html and stall_chart_detail.html: linked event name is now the primary `<h1>` page anchor (id="header-event-name"); secondary meta line (id="header-meta") carries page-context + dates ("Editing Reservation #44 · May 8–10, 2026" on edit page; "Stall Chart for Reservation #44 · May 8–10, 2026" on stall chart). Change Event button triggers inline typeahead expansion (header-typeahead div, display toggle, no animation); filterEventOptions() live-filters _linkedEvents seed array (5 dummy events: Perry GA current + 4 others); selectLinkedEvent() updates h1 + meta + collapses typeahead; cancelChangeEvent() collapses without change. No "Unlink" action — reservation requires a linked event at all times. #card-linked-event section card + all 14 scoped CSS classes (event-search, event-linked-*, event-unlink-icon etc.) deleted from edit_reservation_page.html. .plugin-subtitle rule removed from both files. DS-1 note: header font inherits Space Grotesk from existing .plugin-title CSS — reconcile against Josefin Sans lock at DS-1. Typeahead behavior is mockup-only; real event-source query wiring is C10/plugin-port work. Commit b54e828. edit_reservation_page.html 2486 → 2495 lines; stall_chart_detail.html 779 → 851 lines.

**Last updated (prev):** 2026-05-29 — **C8.H mockup** — Customer Selection toggle deleted entirely from both Stall and RV mapped-content wrappers. Inventory Mode now determines customer experience absolutely: Bulk = quantity picker at checkout (admin assigns stalls/lots post-purchase on the Stall & RV Charts page); Mapped = customer selects specific stalls/lots from the layout at checkout (Interactive Map ships at C10+; graceful degradation handled at C10 render time). toggleCustomerSelection() deleted. updateRvInventoryDisplay() simplified — selBtn/selMode branch removed, always sums zone-qty inputs in Mapped mode. .badge-coming-soon CSS deleted. Inventory Mode hint text updated for all four cases. Commit 59c0031. 2486 lines.

**Last updated (prev):** 2026-05-29 — **C8.G mockup** — Full inventory model restructure. Replaced "Stall Charts" toggle + "Stall Selection Mode" toggle with "Inventory Mode (Bulk | Mapped)" + "Customer Selection (Quantity Picker | Interactive Map)" on both Stall and RV sides. stall-mapped-content wraps row-builder + blocked stalls + stall map upload + Customer Selection. rv-mapped-content wraps zone list (now with "+ Nightly" / "+ Weekend" surcharge labels + surcharge hint) + lot row-builder + blocked lots + Customer Selection. Interactive Map button carries "Coming soon" badge (.badge-coming-soon). New JS: stallMappedIsActive(), rvMappedIsActive(), toggleInventoryMode(), toggleCustomerSelection(). Removed: toggleMode(), toggleLotMode(), addStallRow(), deleteStallRow(), stallChartsIsOn(), rvChartsIsOn(). Updated: updateStallInventoryDisplay() + updateRvInventoryDisplay() use new detection functions; toggleSwitchRow() charts special-case lines removed; DOMContentLoaded stall qty tbody listener removed. CSS selectors #stall-charts-content / #rv-lot-charts-content retargeted to new IDs. Commit 5512e7c. 2550 lines.
