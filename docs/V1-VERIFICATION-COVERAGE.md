...
| **Hosted order page** pricing content (maximal order) | 🟡 | ✅ **CLOSED** 2.7.739 — `hosted-receipt-reconcile-smoke`: a seeded stall+RV composite renders the hosted + PDF receipt (`build_receipt_html`), and the RENDERED grand reconciles to the canonical `compose_order_totals` grand — unadjusted AND after an admin-added custom line item (both surfaces track the new grand + show the line description). Guards the "displayed ≠ actual" risk on the customer receipt. No bug found. |
